...
physeq_read_counts = psmelt(physeq_read_counts_organ) %>%
...
For_barplot[nrow(For_barplot)+1, ] = list("V. amurensis Leaf", sum(For_barplot[match(V_amur_organ[V_amur_organ %in% grep("Leaf", V_amur_organ, value=TRUE)], For_barplot$Sample),"Sum"]))
For_barplot[nrow(For_barplot)+1, ] = list("V. amurensis Stem", sum(For_barplot[match(V_amur_organ[V_amur_organ %in% grep("Stem", V_amur_organ, value=TRUE)], For_barplot$Sample),"Sum"]))
...
Barplot = physeq[, names(physeq) %in% c(cultivars_organ, V_amur_organ)]

Barplot['V. amurensis Leaf'] = rowMeans(Barplot[,names(Barplot) %in% grep("Leaf", V_amur_organ, value=TRUE)])
Barplot['V. amurensis Stem'] = rowMeans(Barplot[,names(Barplot) %in% grep("Stem", V_amur_organ, value=TRUE)])
Barplot = Barplot[, c(cultivars_organ, 'V. amurensis Leaf', 'V. amurensis Stem')]
...
tree = Barplot %>%
	select(Class, Sample, Abundance) %>%
	filter(Sample %in% cultivars_organ)
...
Above_barplot = filter(For_barplot, Sample %in% c(tree$labels$label, 'V. amurensis Leaf', 'V. amurensis Stem'))
Above_barplot[] = Above_barplot[match(c(tree$labels$label, 'V. amurensis Leaf', 'V. amurensis Stem'), Above_barplot$Sample),]
...
Barplot$Sample = factor(Barplot$Sample, levels = c(tree$labels$label, 'V. amurensis Leaf', 'V. amurensis Stem'))
...
# Figure 3(a) Cultivars and V. amurensis ITS metagenome bar plot
ggplot(Barplot) +
...
#	xlab("Sample ID") +
	...
	axis.text.x = element_text(size=30, angle=-30, vjust = 2, hjust = 0, face = "italic"),
    ...
    geom_text(aes(x = Sample, y = For_sum, label = Sum), inherit.aes = FALSE, size = 7) +
...
ggsave("Figure 3(a) Cultivars and V. amurensis ITS metagenome bar plot.png", width = 25, height = 20)
#